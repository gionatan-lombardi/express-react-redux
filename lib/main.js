...
function middleware(options = {}) {
  ...
  function defaultRenderHtml({ appHtml, state, indexJsCompiler }) {
    let html;
    if (productionMode) {
      html = fs.readFileSync(path.resolve(options.dirBuildClient, options.fileIndexHtml), 'utf8');
    } else {
      html = indexJsCompiler.outputFileSystem.readFileSync(path.join(indexJsCompiler.outputPath, options.fileIndexHtml), 'utf8');
    }
    const appDiv = '<div id="app">';
    html = html.replace(appDiv, appDiv + appHtml);
    const preloadedState = `<script>window.__PRELOADED_STATE__=${serialize(state)}</script>`;
    ...
    const render = (err) => {
      ...
      if (context.url) {
        ...
        const html = renderHtml({
          ...
          indexJsCompiler: getCompiler(options.fileIndexJs),
        });
        res.set('content-type', 'text/html');
        res.send(html);
      }
